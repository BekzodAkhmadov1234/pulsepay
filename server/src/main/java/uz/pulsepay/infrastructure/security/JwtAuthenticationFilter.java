 ...
 * The domain {@link uz.pulsepay.identity.domain.model.User} record is <em>never</em> loaded
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest  request,
        ...
        if (rawToken != null) {
            // Only attempt authentication when a Bearer token is actually present.
            // Requests without a token are passed through; protected routes will
            // then be rejected by Spring Security's authorization rules.
    ...
    /**
     * Strips the {@value #BEARER_PREFIX} prefix from the {@code Authorization} header.
     *
     * @return the raw token string, or {@code null} if the header is absent or malformed
     */
    ...
    /**
     * Validates the token and populates the {@link SecurityContextHolder} on success.
     *
     * <p>Any exception thrown by {@link JwtService#extractAllClaims(String)} is caught here
     * so that a bad token results in an unauthenticated pass-through rather than a
    ...
    private void populateSecurityContext(String rawToken) {
        try {
            ...
            // Build Spring Security's built-in User as the principal.
            //
            // phoneE164 is the human-readable authenticated identity (username).
            // kycLevel drives fine-grained authorization: "ROLE_BASIC", "ROLE_VERIFIED", etc.
            //
            // NOTE: this is org.springframework.security.core.userdetails.User — the Spring
            // Security utility class — NOT the domain model uz.pulsepay.identity.domain.model.User.
            // The domain model is entirely decoupled from the security pipeline.
            ...
            // Token is expired, signature is invalid, or the payload is malformed.
            // Proceed unauthenticated; Spring Security's route authorization will reject the request.
